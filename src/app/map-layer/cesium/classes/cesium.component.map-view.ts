...
export class CesiumMapView{
  ...
  setQueryBoundsOnNavigationEnd: (NavigationEnd) => void = (event:NavigationEnd):void => {
    ...
    this.cesium.router.navigateByUrl(urlTree.toString());
  };
  ...
  moveEnd(e?):Promise<boolean> {
    ...
    let navigationExtras:NavigationExtras = this.cesium.queryParamsHelperService.getQuery({lng, lat, height, heading, pitch, roll, mode3d, markers, rotate, layers});
